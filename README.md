...
- 2024.03–2024.07: CNN과 ResNet18을 이용한 차선 손상 등급 분류 (개인 진행)
- 2024.04: 같은 프로젝트에 참여한 팀원들과 픽셀 단위 손상 영역 라벨링(LabelMe) 공동 진행
- 2026.09: 다른 서버에 남아있던 위 라벨 데이터를 다시 확보해 U-Net 손상 영역 분할 모델 학습 (개인 진행)

라벨 제작 시점과 모델 학습 시점 사이에 텀이 있는 이유는, 2024년에 만들어둔 라벨 데이터를 이후 접근하지 못하고 있다가 2026년에 다시 찾았기 때문입니다. 두 실험의 데이터와 수행 시점이 다르기 때문에 하나의 연속된 실험 결과로 합치지 않았습니다.
...
## 2026년 추가 실험: 픽셀 단위 손상 영역 분할

등급 분류 결과만으로는 손상 위치를 확인할 수 없습니다. 이 프로젝트를 같이 진행한 팀원들과 2024년 4월에 LabelMe로 차선 영역과 개별 손상 부위를 폴리곤으로 라벨링해뒀었는데, 당시엔 다른 서버에 있던 데이터라 활용하지 못했습니다. 2026년 9월에 데이터를 다시 확보해서, 이를 이진 마스크로 변환해 U-Net을 학습했습니다.
...
예측 마스크에서 손상 픽셀의 비율을 계산하면 점검할 도로 구간을 먼저 고르는 데 사용할 수 있습니다. 현재 실험은 제한된 이미지로 진행했으며, 실제 도로에서 사용하려면 촬영 환경이 달라졌을 때의 성능과 오탐률을 추가로 확인해야 합니다.

---

## 두 실험의 차이

등급 분류는 이미지마다 라벨 하나만 필요하지만 손상 위치는 알 수 없습니다. 세그멘테이션은 폴리곤 라벨을 만드는 시간이 더 들지만 손상의 위치와 면적을 계산할 수 있습니다. 분류 데이터는 A/B/C/D, 세그멘테이션 데이터는 A/C/F 체계이며 이미지 집합도 서로 다릅니다.
